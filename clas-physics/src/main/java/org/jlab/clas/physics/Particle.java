...
public class Particle {
    ...
    public double px(){ return this.vector().px();}
    public double py(){ return this.vector().py();}
    public double pz(){ return this.vector().pz();}
    public double p(){ return this.vector().p();}
    public double theta(){ return this.vector().theta();}
    public double phi(){ return this.vector().phi();}
    ...
    {
        if(part.vector().p()==0||this.vector().p()==0) return -1;
    ...
    {
        if(pname.compareTo("mass")==0)  return partVector.mass();
        if(pname.compareTo("mass2")==0)  return partVector.mass2();
        if(pname.compareTo("theta")==0) return partVector.theta();
        if(pname.compareTo("phi")==0)   return partVector.phi();
        if(pname.compareTo("p")==0)   return partVector.p();
        if(pname.compareTo("mom")==0)   return partVector.p();
        if(pname.compareTo("e")==0)     return partVector.e();
        if(pname.compareTo("px")==0)    return partVector.px();
        if(pname.compareTo("py")==0)    return partVector.py();
        if(pname.compareTo("pz")==0)    return partVector.pz();
        if(pname.compareTo("vx")==0) return partVertex.x();
        if(pname.compareTo("vy")==0) return partVertex.y();
        if(pname.compareTo("vz")==0) return partVertex.z();
        if(pname.compareTo("vertx")==0) return partVertex.x();
        if(pname.compareTo("verty")==0) return partVertex.y();
        if(pname.compareTo("vertz")==0) return partVertex.z();
        
        System.out.println("[Particle::get] ERROR ----> variable "
    ...
    public String toLundString(){
        StringBuilder str = new StringBuilder();
        str.append(String.format("%3.0f. %4d %6d %2d %2d %9.4f %9.4f %9.4f ", 
                (float) particleCharge, (int) 1,
                particleID, (int) 0, (int) 0,
                partVector.px(),partVector.py(),partVector.pz()));
        
        str.append(String.format("%9.4f %9.4f %11.4f %9.4f %9.4f", 
                partVector.e(),partVector.mass(),
                partVertex.x(),partVertex.y(),partVertex.z()
                ));
        /*
        str.append(String.format("%6d %3d %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f %9.5f",
                particleID,particleCharge,partVector.mass(),
                partVector.px(),partVector.py(),partVector.pz(),
                partVector.p(),
                Math.toDegrees(partVector.theta()),
                Math.toDegrees(partVector.phi()),
                partVertex.x(),partVertex.y(),partVertex.z()
                ));*/
        return str.toString();
    }
    ...
    {
        this.partVector.setPxPyPzM(part.vector().px(), 
                part.vector().py(),
                part.vector().pz(),part.vector().mass());
        this.partVertex.setXYZ(part.vertex().x(),
    ...
    public void combine(Particle cpart, int sign)
    {
        ...
        this.partVertex.setXYZ(cpart.vertex().x(), 
                cpart.vertex().y(),cpart.vertex().z()
                );
        //} else {
            /*
            Line3D pl = new Line3D();
            Line3D pn = new Line3D();
            
            pl.setOrigin(partVertex.x(),
                    partVertex.y(),
                    partVertex.z()
                    );
            
            pl.setEnd(partVertex.x()+partVector.vect().x(), 
                    partVertex.y()+partVector.vect().y(),
                    partVertex.z()+partVector.vect().z());
            ...
            pn.setEnd(
                    cpart.vertex().x() + cpart.vector().px(),
                    cpart.vertex().y() + cpart.vector().py(),
                    cpart.vertex().z() + cpart.vector().pz()                
                    );
            Line3D   doca  = pl.distance(pn);
            Point3D  docam = doca.middle();
            this.partVertex.setXYZ(docam.x(), docam.y(), docam.z());
        
            /*
             * partVertex.setXYZ(cpart.vertex().x(),
                cpart.vertex().y(),cpart.vertex().z());
        */
    }
    ...
    public Vector3 particleDoca(Particle cpart)
    {
        ...
        Point3D  docam = doca.middle();
        //this.partVertex.setXYZ(docam.x(), docam.y(), docam.z());
        return new BasicVector(docam.x(),docam.y(),docam.z());*/
        return new Vector3();
    }
