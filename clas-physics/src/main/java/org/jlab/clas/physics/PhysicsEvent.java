...
public class PhysicsEvent {
    ...
    public int countByCharge(int charge)
    {
        ...
        for(int loop = 0; loop < eventParticles.size(); loop++)
        {
            ...
            if(eventParticles.get(loop).charge()==charge) icount++;
        }
    ...
    public void addParticle(int pid, double px, double py, double pz, double vx, double vy,
            double vz)
    {
    ...
    public void setBeam(String particle, double mom_z){
        ...
        if(partInfo!=null){
            this.eventBeam.setVector(partInfo.pid(), 
                    0.0, 0.0, mom_z, 0.0, 0.0, -100.0);
        }
    }
    
    public void setBeam(double mom_z){
            this.eventBeam = new Particle(11, 
                    0.0, 0.0, mom_z, 0.0, 0.0, -100.0);
    }
    ...
    public Particle beamParticle() {return eventBeam;}
    
    public Particle targetParticle() {return eventTarget;}
    ...
    public void removeParticleByPid(int pid,int skip)
    {
        ...
        if(index<0||index>=this.count()){
            System.out.println("----> error. paritcle does not exist pid="
                    + pid + " skip="+skip);
        } else {
    ...
    {        
        ...
        for(int loop=0;loop<eventParticles.size();loop++)
        {
            ...
            {
                if(skip==skiped) return eventParticles.get(loop);
                else skiped++;
            } 
        }
        return null;
    ...
    {        
        if(pid==5000) return this.beamParticle();
        if(pid==5001) return this.targetParticle();
    ...
    public Particle closestParticle(Particle child){
        ...
        if(index>=0) part.copyParticle(eventParticles.get(index));
        return part;
    }
    ...
    public List  getParticleListByPid(int... pids_order){
        ...
        for(int loop = 0; loop < pids_order.length; loop+=2){
            Particle part = this.getParticleByPid(pids_order[loop], 
                    pids_order[loop+1]);
            if(part!=null){
                plist.add(part);
            }
